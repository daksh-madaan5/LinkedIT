...
export const OrdersTable: React.FC<OrdersTableProps> = ({
  ...
  if (jobs.length === 0) {
    return (
      <div className="pt-10 pb-6 text-center text-slate-400 text-xs italic">
        No delivery orders added yet. Add deliveries in the left panel or load the Bhubaneswar Demo.
      </div>
    );
  }
  ...
  return (
    <div className="w-full h-full flex flex-col justify-between overflow-hidden bg-white">
      <div className="flex-1 overflow-auto">
        <table className="w-full text-left border-collapse text-xs">
          <thead className="bg-slate-50 sticky top-0 z-10 border-b border-slate-200 text-slate-700 select-none">
            <tr>
              <th className="py-2 px-3 font-semibold text-left w-20">Order ID</th>
              <th className="py-2 px-3 font-semibold text-left w-24">Status</th>
              <th className="py-2 px-3 font-semibold text-left w-32">Vehicle</th>
              <th className="py-2 px-3 font-semibold text-center w-16">Stop #</th>
              <th className="py-2 px-3 font-semibold text-left">Location</th>
              <th className="py-2 px-3 font-semibold text-right w-20">Demand</th>
              <th className="py-2 px-3 font-semibold text-right w-24">Service Time</th>
              <th className="py-2 px-3 font-semibold text-center w-28">Time Window</th>
              <th className="py-2 px-3 font-semibold text-center w-24">Est. Arrival</th>
              <th className="py-2 px-3 font-semibold text-center w-24">Est. Depart</th>
              <th className="py-2 px-3 font-semibold text-center w-16">Priority</th>
            </tr>
          </thead>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-2 px-3 text-left">
                      {!response ? (
                        <span className="inline-flex items-center text-[11px] text-slate-700 bg-slate-100 px-2 py-0.5 rounded font-medium border border-slate-200/80">
                          Pending
                        </span>
                      ) : isUnassigned ? (
                        <span className="inline-flex items-center text-[11px] text-amber-700 bg-amber-50 px-2 py-0.5 rounded font-medium border border-amber-200/80">
                          Unassigned
                        </span>
                      ) : (
                        <span className="inline-flex items-center text-[11px] text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded font-medium border border-emerald-200/80">
                          Assigned
                        </span>
                      )}
                    </td>
                    ...
                    <td className="py-2 px-3 text-left font-mono text-slate-600 text-[11px]">
      ...
      {/* Table Footer */}
      <div className="px-3.5 py-1.5 bg-slate-50 border-t border-slate-200 text-[11px] text-slate-500 font-sans flex items-center justify-between shrink-0">
